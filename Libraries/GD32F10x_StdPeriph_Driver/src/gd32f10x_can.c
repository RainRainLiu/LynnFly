...
/**
  ...
  * @param  NewValue: new state of the CAN peripheral. This parameter can 
...
void CAN_StopTransmit(CAN_TypeDef* CANx, uint8_t Mailbox)
{
    ...
    switch (Mailbox)
    {
    case (CAN_TXMAILBOX_0): CANx->TSTR |= CAN_TSTR_MST0;
        break;
    case (CAN_TXMAILBOX_1): CANx->TSTR |= CAN_TSTR_MST1;
        break;
    case (CAN_TXMAILBOX_2): CANx->TSTR |= CAN_TSTR_MST2;
        break;
    default:
        break;
    }
...
void CAN_ClearBitState(CAN_TypeDef* CANx, uint32_t CAN_FLAG)
{
    ...
    if (CAN_FLAG == CAN_FLAG_ET) /* ER register */
    {
        /* Clear the selected CAN flags */
        CANx->ER = (uint32_t)RESET;
    }
    else /* STR or TSTR or RFR0 or RFR1 */
    {
