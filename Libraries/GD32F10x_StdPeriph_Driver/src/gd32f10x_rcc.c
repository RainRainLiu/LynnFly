...
/**
  ...
  * @param  RCC_OTGCLK: specify the USB_OTG clock source. This clock is derived 
  *         from the PLL.
  *          This parameter can be one of the following values:
  ...
  * @retval None
  */
void RCC_OTGFSCLKConfig(uint32_t RCC_OTGCLK)
{ 
    
    /* Clear OTGPS bit */
    RCC->GCFGR &= ~RCC_GCFGR_OTGFSPS;
    /* Set OTGPS bits according to RCC_OTGCLK value */
    RCC->GCFGR |= RCC_OTGCLK;
