...
//    /* Keys configuration */
//    GD_EVAL_KeyInit(KEY_RIGHT, KEY_MODE_EXTI);
//    GD_EVAL_KeyInit(KEY_LEFT, KEY_MODE_EXTI);
...
void  USB_HWP_ClockConfig (void)
{
    ...
    RCC_USBCLKConfig(RCC_USBCLK_PLL_DIV1);
...
void  USB_HWP_DeTnit (void)
{

    _SetCTLR(CTLR_CLOSE);  //¹Ø±ÕUSBµçÔ´

    NVIC->ICER[USB_LP_CAN1_RX0_IRQn >> 0x05] = (uint32_t)0x01 << (USB_LP_CAN1_RX0_IRQn & (uint8_t)0x1F);

    RCC->APB1CCR &= ~RCC_APB1PERIPH_USB;
